...
function refreshAtividadeManual() {
    var request = new XMLHttpRequest();
    var vBoard = document.getElementById("InformacaoAtividadeManual");
    request.onload = function () {
        vBoard.innerHTML = this.responseText;
        vBoard.style.color = "white";
        console.log(this.responseText);
        setTimeout(refreshAtividadeManual, 2000);
    };

    request.ontimeout = function () {
        vBoard.innerHTML = "Server timeout, still trying ...";
        vBoard.style.color = "red";
        setTimeout(refreshAtividadeManual, 100);
    };

    request.onerror = function () {
        vBoard.innerHTML = "No server reply, still trying ...";
        vBoard.style.color = "red";
        setTimeout(refreshAtividadeManual, 5000);
    };
    request.open("GET", "/InformacaoAtividadeManual", true);
    request.timeout = 5000;
    request.send();
}